...
import { notifyBackgroundRuns } from "@/lib/background-runs";
import { resolveToolActivity, ACTIVITY_LABELS } from "@/lib/activity-labels";
import {
  deriveDocumentsFromMessages,
  buildCitationDocumentsMap,
...
type StreamContextType = ReturnType<typeof useStream<StateType>> & {
  ...
  stop: () => void;
  // --- Hoisted per-message derivations ---
  // Computed ONCE per messages change up here, so each message bubble renders
  // with O(1) lookups instead of re-scanning the whole thread on every streamed
  // token (which made per-token work O(n²) on long threads).
  // Activity steps for every COMPLETED turn, keyed by its answer message id.
  historicalStepsMap: Map<string, ThinkingStep[]>;
  // Cumulative citation-source set per message id (own set, else the nearest
  // preceding answer's) — what inline [n] markers resolve against.
  citationDocumentsMap: Map<string, AnalyzedDocument[]>;
  // True until the thread has any AI/tool message (gates the interrupt view).
  hasNoAIOrToolMessages: boolean;
  // Last AI message id while a run is in flight — the host for the LIVE
  // activity disclosure. Unlike streamingMessageId it does NOT require the
  // message to be the thread tail, so it stays on the answer message across
  // trailing tool results.
  liveHostAiId: string | null;
};
...
// Per-answer-id steps for ALL completed turns (this session or opened from
// history) in one O(n) pass: partition messages into turns at each `human`
// message, key each turn's steps by its answer — the turn's LAST AI message,
// sliced inclusive (trailing tool results excluded). Built in the provider once
// per messages/timestamps change so each AssistantMessage does an O(1) lookup
// per render instead of re-deriving its whole turn on every streamed token.
function buildHistoricalStepsMap(
  messages: Message[],
  times: MessageTimes,
  ...
  const flushTurn = (endExclusive: number) => {
    let answerIdx = -1;
    ...
    const answerId = answerIdx >= 0 ? messages[answerIdx]?.id : undefined;
    if (!answerId) return;
    const steps = deriveStepsFromTurn(
      messages.slice(turnStart, answerIdx + 1),
      times,
      turnStartTime(messages, humanIdx, times),
      false,
    );
    if (steps.length) map.set(answerId, steps);
  };
...
export const StreamProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const stream = useStream<StateType>({
    ...
    // Coalesce same-tick SSE events into ONE React render per macrotask instead
    // of one render per event (a single network chunk often carries a token
    // event + values snapshot + custom event). NOTE: `true` is deliberate — a
    // NUMBER here acts as a trailing debounce that resets on every event, which
    // can starve renders entirely on a fast stream.
    throttle: true,
    // Persist run metadata (sessionStorage) so a page refresh or remount during
    // a run re-joins the live stream instead of losing the partial answer. This
    // also makes every submit stream-resumable by default (the SDK derives
    // streamResumable + onDisconnect:"continue" from it), so a transient
    // network drop no longer cancels the run server-side.
    ...
    onCreated: (run) => {
      ...
      // The SDK just persisted this run's `lg:stream:<threadId>` metadata —
      // light up the "running" badge in the history sidebar immediately, and
      // refresh the thread list so a brand-new conversation shows up there
      // while it is still streaming (not only after it finishes).
      notifyBackgroundRuns();
    ...
    onFinish: (state) => {
      ...
      // buildHistoricalStepsMap below), so nothing needs sealing here.
      pendingStepsRef.current = [];
      setThinkingSteps([]);
  ...
  const stop = useCallback(async () => {
    setStopped(true);
    stream.stop();
    // stop() removed the SDK's lg:stream key for this thread synchronously —
    // clear the sidebar badge right away rather than waiting for isLoading.
    notifyBackgroundRuns();
  ...
  // stopped so it doesn't interfere with future turns. Every way a stream ends
  // in the active thread (finish / error / stop / rejoin drained) passes
  // through here, so it doubles as the catch-all re-sync point for the
  // background-run badges (the SDK removes its lg:stream key just before).
  useEffect(() => {
    if (!stream.isLoading) {
      setStopped(false);
      notifyBackgroundRuns();
  ...
  // Per-thread UI hygiene: when switching to a DIFFERENT conversation, the
  // per-turn state (live steps, pending documents/debug, follow-ups, run id,
  // stop/error flags) belongs to the previous thread — drop it so it can't
  // leak into the newly opened one. The null -> id transition is skipped: that
  // is the SAME session acquiring its URL when the first turn of a new chat
  // creates the thread. The in-flight run is NOT cancelled — submits are
  // stream-resumable with onDisconnect:"continue", so it keeps executing
  // server-side (tracked by the background-run registry) and re-opening the
  // thread rejoins its live stream, replaying custom events to rebuild the
  // live trace.
  const prevThreadIdRef = useRef<string | null | undefined>(undefined);
  useEffect(() => {
    const prev = prevThreadIdRef.current;
    prevThreadIdRef.current = threadId;
    if (prev === undefined || prev === null || prev === threadId) return;
    resetTurnState();
    setStopped(false);
    setLastDonePayload(null);
    setLiveThoughtExpanded(false);
    lastSeenAiIdRef.current = null;
  }, [threadId, resetTurnState]);
  ...
  const messageTimes = useMemo<MessageTimes>(() => {
    ...
    return map;
    // NOT keyed on `stream`: the SDK returns a NEW handle object every render,
    // so that dependency would recompute this O(n²) pass on literally every
    // render and cascade fresh identities into all downstream activity memos.
    // created_at only changes when the messages array changes identity — token
    // updates, thread load, branch switch, AND the post-run history refetch
    // (the SDK clears stream values to the refetched history then, verified in
    // manager.js) — with isThreadLoading as cheap insurance for initial load.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [stream.messages, stream.isThreadLoading]);
  ...
  // --- Hoisted per-message derivations (see StreamContextType docs) ---
  const historicalStepsMap = useMemo(
    () => buildHistoricalStepsMap(stream.messages, messageTimes),
    [stream.messages, messageTimes],
  );
  const citationDocumentsMap = useMemo(
    () => buildCitationDocumentsMap(stream.messages, mergedDocumentsMap),
    [stream.messages, mergedDocumentsMap],
  );
  const hasNoAIOrToolMessages = useMemo(
    () => !stream.messages.some((m) => m.type === "ai" || m.type === "tool"),
    [stream.messages],
  );
  const liveHostAiId = useMemo(
