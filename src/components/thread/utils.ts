...
/**
 * Whether the answer text carries at least one inline `[n]` citation that
 * resolves to a referenced document — i.e. the answer actually cites a source.
 *
 * Gates the "Referenced Sources" panel: the source set is CUMULATIVE, so a turn
 * that renders text WITHOUT citing anything (a follow-up reformat that ran no
 * search, or a plain reply) would otherwise still show the whole running list
 * carried forward. We only surface the panel when the answer visibly points into
 * it. Mirrors `linkifyCitations`' validity rule: only a `[n]` whose number
 * matches a document `index` counts — a hallucinated/invalid marker (which
 * linkifyCitations drops) is not a citation and must not reveal the panel.
 */
export function hasInlineCitation(
  content: string,
  documents?: { index?: number }[],
): boolean {
  if (!content || !documents || documents.length === 0) return false;
  const validIndices = new Set<number>();
  documents.forEach((d, i) => validIndices.add(d.index ?? i + 1));
  const re = /\[(\d+)\]/g;
  let match: RegExpExecArray | null;
  while ((match = re.exec(content)) !== null) {
    if (validIndices.has(Number(match[1]))) return true;
  }
  return false;
}

/**
 * Resolve the CUMULATIVE referenced-source set for every message id in ONE
 * forward pass — used both to render the "Referenced Sources" panel and to link
 * inline `[n]` citations. The backend numbers documents append-only across the
 * whole conversation, so an answer's OWN retrieved set (`documentsMap[id]`) is
 * already the running list as of that turn. A follow-up turn that
 * reformats/expands WITHOUT running its own KB search has no set of its own, so
 * it falls back to the nearest PRECEDING message's non-empty set — that lets any
 * carried-forward `[n]` markers still resolve to links instead of rendering as
 * raw brackets, and (only when that answer itself carries such a marker) feeds
 * the panel with the running list. Messages before the conversation's first
 * search resolve to nothing and simply have no entry. A no-search follow-up
 * whose text cites nothing renders no panel — the render is gated on
 * `hasInlineCitation` (see `showReferencedSources` in ai.tsx), not on this set
 * being non-empty.
 *
 * Built once per messages/documents change in the provider so each message
 * render does an O(1) lookup instead of an O(n) backward scan on every
 * streamed token.
 */
export function buildCitationDocumentsMap(
  messages: Message[],
  documentsMap: Record<string, AnalyzedDocument[]>,
): Map<string, AnalyzedDocument[]> {
  const map = new Map<string, AnalyzedDocument[]>();
  let carry: AnalyzedDocument[] | undefined;
  for (const m of messages) {
    if (!m.id) continue;
    const own = documentsMap[m.id];
    const resolved = own && own.length > 0 ? own : carry;
    if (resolved) map.set(m.id, resolved);
    if (own && own.length > 0) carry = own;
  }
  return map;
}
