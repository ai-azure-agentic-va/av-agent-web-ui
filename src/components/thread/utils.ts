...
/**
 * Resolve the CUMULATIVE referenced-source set for an answer — used both to render
 * the "Referenced Sources" panel and to link its inline `[n]` citations. The
 * backend numbers documents append-only across the whole conversation, so an
 * answer's OWN retrieved set (`documentsMap[messageId]`) is already the running
 * list as of that turn. A follow-up turn that reformats/expands WITHOUT running its
 * own KB search has no set of its own, so we fall back to the nearest PRECEDING
 * answer that did retrieve documents — that keeps the (append-only) panel visible
 * and lets carried-forward `[n]` markers still resolve instead of rendering as raw
 * brackets. Returns undefined only before the conversation's first search.
 */
