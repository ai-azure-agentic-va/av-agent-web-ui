...
export function AssistantMessage({
  ...
  // list, so the collapsed "Referenced Sources" panel stays visible (append-only)
  // and any carried-forward [n] markers still link. Feeds both the panel and the
        ...
        {isToolResult ? (
          ...
          <>
            ...
            {/* Cumulative "Referenced Sources" (append-only, collapsed by
                default), shown at the end of every answer that carries text —
                including a follow-up that ran no search of its own, which carries
                the running list forward. Gated on text so a mid-stream
                tool-call-only bubble never shows it. */}
            {contentString.length > 0 &&
              citationDocuments &&
